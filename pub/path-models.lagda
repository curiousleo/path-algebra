We now discuss three models---or inhabitants---of the \AgdaRecord{SobrinhoAlgebra} record to demonstrate that they exist, that Sobrinho Algebras are not categorical (i.e. are not inhabitable by only one structure up to isomorphism), and to use later in Section~\ref{sect.example} where we provide an example execution of our algorithm.

Trivially, the axioms of a \AgdaRecord{SobrinhoAlgebra} are satisfied by the unit type, \AgdaDatatype{⊤}.
Defining a degenerate `addition' operation on \AgdaDatatype{⊤}, we inhabit \AgdaRecord{SobrinhoAlgebra} by taking the algebra's addition and multiplication to be this operation and its two unit elements to be \AgdaInductiveConstructor{tt}, the unit value.
The axioms of the \AgdaRecord{SobrinhoAlgebra} are easily satisfied.
...
More useful models for \AgdaRecord{SobrinhoAlgebra} can be obtained as follows.
