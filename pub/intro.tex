...
Further, our algorithm is parameterised by a non-distributive algebraic structure, which we call a `Sobrinho Algebra', which distills the algebraic properties assumed in previous work on this topic into a concrete algebraic structure.
The correctness proof shows that our algorithm does indeed compute the Right Local solution to the fixpoint equation mentioned above, given a graph's adjacency matrix over a Sobrinho Algebra, and we believe that this is the first axiom-free correctness proof of a shortest path algorithm using the `algebraic approach', rather than follow more traditional proof strategies.
Our implementation, and proof of correctness, are carried out in Agda, and we make extensive use of Agda's dependent types, and some more cutting edge features of the language---such as induction-recursion---to structure and complete the proof.
