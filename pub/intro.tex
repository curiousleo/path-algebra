...
Algebraically, Dijkstra's Algorithm relies crucially on the distributivity axioms of the underlying semiring, mentioned above, to compute \emph{globally optimal} paths.
However, recent work on algebraically modelling the Border Gateway Protocol (BGP), the Internet routing protocol which maintains connectivity between Internet Service Providers, has led to the idea of \emph{local} optimal paths (also called equilibrium solutions) where the underlying algebraic structure is non-distributive~\cite{sobrinho_routing_2010}.
In this relaxed setting non-distributive path algebras are used in favour of semirings, and Dijkstra's Algorithm
